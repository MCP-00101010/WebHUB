// Vendored from @cloudflare/speedtest 1.13.0 (MIT).
// Exposed as a classic-script global for file:// Hub compatibility.
...
const resolveSpeedTestUrl = (apiUrl) => {
	try {
		return new URL(apiUrl);
	} catch {
		const origin = window.location.origin;
		const baseUrl = origin && origin !== "null" ? origin : window.location.href;
		return new URL(apiUrl, baseUrl);
	}
};
const withAuthorizationToken = (apiUrl, token) => {
	if (!token) return apiUrl;
	const urlObj = resolveSpeedTestUrl(apiUrl);
...
const redactAuthorizationToken = (apiUrl) => {
	let urlObj;
	try {
		urlObj = resolveSpeedTestUrl(apiUrl);
	} catch {
		return apiUrl;
	}
...
//#endregion
globalThis.CloudflareSpeedTest = SpeedTestEngine;
