...
function _universalSearchProviders(widget) {
  ...
  const defaults = [
    { id: 'google', name: 'Google', alias: 'g', icon: 'G', template: 'https://www.google.com/search?q={query}' },
    { id: 'youtube', name: 'YouTube', alias: 'yt', icon: '▶', template: 'https://www.youtube.com/results?search_query={query}' },
    { id: 'imdb', name: 'IMDb', alias: 'imdb', icon: 'IMDb', template: 'https://www.imdb.com/find/?q={query}' }
  ];
...
function _universalSearchRender(widget, element, context) {
  ...
  const input = document.createElement('input'); input.type = 'search'; input.placeholder = 'Search the Hub, enter a URL, or use g, yt, imdb…'; input.value = runtime.query; input.autocomplete = 'off'; input.setAttribute('aria-label', 'Universal search');
...
function _universalSearchRenderSettings(widget, container) {
  ...
  container.innerHTML = `<div class="universal-search-settings-list"></div><button type="button" class="secondary-btn universal-search-add">Add provider</button>
  ...
  const renderRows = () => {
    ...
    providers.forEach((provider, index) => {
      ...
      const fields = document.createElement('div'); fields.className = 'universal-search-settings-grid';
      [['name','Name'],['alias','Alias'],['icon','Icon'],['template','https://…{query}']].forEach(([key, placeholder]) => { const input = document.createElement('input'); input.className = 'settings-text-input'; input.placeholder = placeholder; input.value = provider[key]; input.addEventListener('input', () => { provider[key] = input.value; if (key === 'name') renderDefault(); }); fields.appendChild(input); });
      const remove = document.createElement('button'); remove.type = 'button'; remove.className = 'icon-btn is-danger'; remove.textContent = '×'; remove.addEventListener('click', () => { providers.splice(index, 1); renderRows(); renderDefault(); }); row.append(fields, remove); list.appendChild(row);
    });
  ...
  container.querySelector('.universal-search-add').addEventListener('click', () => { providers.push({ id: _universalSearchProviderId(), name: 'Search', alias: '', icon: '⌕', template: 'https://example.com/search?q={query}' }); renderRows(); renderDefault(); });
